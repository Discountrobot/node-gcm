...
Sender.prototype.sendNoRetry = function(message, recipient, callback) {
    ...
    var post_req = req(request_options, function (err, res, resBodyJSON) {
        ...
        if (res.statusCode >= 500) {
            err.message = 'GCM service is unavailable (500)';
            debug(err.message);
            return callback(err, null);
        } else if (res.statusCode === 401) {
            err.message = 'Unauthorized (401). Check that your API token is correct.';
            debug(err.message);
            return callback(err, null);
        } else if (res.statusCode !== 200) {
            err.message = 'Invalid request (' + res.statusCode + '): ' + resBodyJSON;
            debug(err.message);
            return callback(err, null);
        }
